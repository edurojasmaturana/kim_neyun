...
# --- GUARD DE VERSIONES ---
# Los .pkl (Scaler/PCA) fueron serializados con scikit-learn 1.6.1 y numpy 2.0.2.
# Cargarlos con otra versión puede: (1) fallar el import de la clase, o peor,
# (2) cargar bien pero PREDECIR DISTINTO en silencio. En un motor clínico eso es inaceptable,
# así que fallamos fuerte y temprano (en el arranque del deploy, no en producción).
# Override de emergencia: KIM_SKIP_VERSION_GUARD=1 (degrada el error a warning).
import sklearn
import warnings as _warnings
from sklearn.exceptions import InconsistentVersionWarning

_VERSIONES_ENTRENAMIENTO = {"scikit-learn": "1.6.1", "numpy": "2.0.2"}
_versiones_actuales = {"scikit-learn": sklearn.__version__, "numpy": np.__version__}
_desajustes = {k: (_VERSIONES_ENTRENAMIENTO[k], v)
               for k, v in _versiones_actuales.items()
               if v != _VERSIONES_ENTRENAMIENTO[k]}

if _desajustes:
    _detalle = ", ".join(f"{k}: esperado {esp} / instalado {act}"
                         for k, (esp, act) in _desajustes.items())
    _msg = (f"Versiones distintas a las usadas al entrenar los .pkl ({_detalle}). "
            "Riesgo de predicciones inconsistentes.")
    if os.getenv("KIM_SKIP_VERSION_GUARD") == "1":
        logger.warning(f"[VERSION GUARD OMITIDO] {_msg}")
    else:
        raise RuntimeError(
            f"{_msg} Aborta el arranque. Alinea requirements.txt o define "
            "KIM_SKIP_VERSION_GUARD=1 para forzar (bajo tu responsabilidad)."
        )

# Cualquier desajuste de versión que joblib detecte al deserializar también debe ser fatal.
if os.getenv("KIM_SKIP_VERSION_GUARD") != "1":
    _warnings.filterwarnings("error", category=InconsistentVersionWarning)
